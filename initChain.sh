...
#personal.newAccount("Write here a good, randomly generated, passphrase!")
#EOF
geth --ws --wsport 8546 --wsorigins all --datadir ~/.ethereum_private init $1
sleep 2
...
bla
EOF
cat << EOF | geth --verbosity 4 --networkid 15 --unlock "0,1" --rpc --cache 512 --ipcpath ~/Library/Ethereum/geth.ipc --datadir ~/.ethereum_private &
bla
bla
EOF